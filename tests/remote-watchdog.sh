...
case "\$args" in
  ...
  *NetworkIn*)  printf '%s\n' '$2' ;;
  *NetworkOut*) printf '%s\n' '$3' ;;
  *stop-instances*) echo stopped >> "$T/stops"; printf '{}\n' ;;
  *) printf 'None\n' ;;
esac
FAKE
  chmod +x "$T/bin/aws"
...
run() { # run <idle-counter-start>
  ...
  echo "$(cat "$T/state/idle" 2>/dev/null)|$(test -f "$T/stops" && echo STOPPED || echo -)"
}
...
mkfake "i-abc	$old" 1000 2000
check "stop issued, counter reset" "$(run 2)" "0|STOPPED"
...
check "stops once the limit is hit" "$(run 2)" "0|STOPPED"
