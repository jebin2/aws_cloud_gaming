...
# A no-op that does NOT read stdin. Reading it hung the whole suite: the script
# calls `logger -t tag "message"` with no pipe, so a stub that cats stdin waits
# on the terminal forever.
printf '#!/usr/bin/env bash\nexit 0\n' > "$T/bin/logger"
...
echo "3. a failed mirror still shuts the box down"
...
check "shuts down anyway" "$(run 3 1)" "PUSH SHUTDOWN "
